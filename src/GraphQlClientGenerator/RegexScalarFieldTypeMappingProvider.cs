...
public class RegexScalarFieldTypeMappingProvider(IReadOnlyCollection<RegexScalarFieldTypeMappingRule> rules) : IScalarFieldTypeMappingProvider
{
    ...
    public static IReadOnlyCollection<RegexScalarFieldTypeMappingRule> ParseRulesFromJson(string json) => JsonSerializer.Deserialize<IReadOnlyCollection<RegexScalarFieldTypeMappingRule>>(json) ?? [];
    ...
    public ScalarFieldTypeDescription GetCustomScalarFieldType(ScalarFieldTypeProviderContext context)
    {
        ...
        foreach (var rule in _rules)
            if (Regex.IsMatch(context.FieldName, rule.PatternValueName) && Regex.IsMatch(context.OwnerType.Name, rule.PatternBaseType) && Regex.IsMatch(valueType.Name ?? String.Empty, rule.PatternValueType))
                return new ScalarFieldTypeDescription { NetTypeName = GenerationContext.GetNullableNetTypeName(context, rule.NetTypeName, rule.IsReferenceType), FormatMask = rule.FormatMask };
